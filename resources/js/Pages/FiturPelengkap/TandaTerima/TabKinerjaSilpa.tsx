...
export default function TabKinerjaSilpa() {
    ...
    const fetchGenerateCount = async (year: string) => {
        try {
            const response = await axios.get(route('api.kinerja-silpa-tanda-terima.check-available'), {
                params: { tahun: year }
            });
    ...
    const fetchYears = async () => {
        try {
            const response = await axios.get(route('api.kinerja-silpa-tanda-terima.tahun'));
            if (response.data.success) {
    ...
    const fetchStats = async () => {
        try {
            const responseAvail = await axios.get(route('api.kinerja-silpa-tanda-terima.check-available'));
    ...
    const fetchData = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axios.get(route('api.kinerja-silpa-tanda-terima.search'), {
                params: {
                    page: page,
                    ...filters
                }
    ...
    const processGenerateBatch = async () => {
        ...
        try {
            ...
            const initialCheck = await axios.get(route('api.kinerja-silpa-tanda-terima.check-available'), {
            ...
            while (hasMore) {
                const response = await axios.post(route('kinerja-silpa-tanda-terima.generate-batch'), {
                    limit: BATCH_SIZE,
    ...
    const processDeleteAll = async (e: React.FormEvent) => {
        ...
        setProcessing(true);
        try {
            const response = await axios.delete(route('kinerja-silpa-tanda-terima.delete-all'));
            if (response.data.success) {
    ...
    const processDownloadAll = (mode: 'current' | 'year') => {
        ...
        window.open(`${route('kinerja-silpa-tanda-terima.download-all')}?${params.toString()}`, '_blank');
    ...
    const processPrint = (settings: PrintSettings) => {
        if (selectedTandaTerimaId) {
            const url = route('kinerja-silpa-tanda-terima.pdf', selectedTandaTerimaId);
            const params = new URLSearchParams({
        ...
        <div>
            <div className="bg-white dark:bg-gray-800 shadow">
                <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                    ...
                    <h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Manajemen Tanda Terima SiLPA</h2>
            ...
            <Modal show={confirmingBatchGenerate} onClose={closeModal}>
                <div className="p-6">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">
                        Generate Tanda Terima SiLPA Otomatis
                    </h2>
            ...
            <Modal show={confirmingDownload} onClose={closeModal}>
                <div className="p-6">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">
                        Download Semua Tanda Terima SiLPA
                    </h2>
            ...
            <Modal show={confirmingDeleteAll} onClose={closeModal}>
                <form onSubmit={processDeleteAll} className="p-6">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">
                        Hapus SEMUA Tanda Terima SiLPA
                    </h2>
            ...
            <PrintSettingsModal
                ...
                title="Pengaturan Cetak Tanda Terima SiLPA"
            />
            ...
            <PdfPreviewModal
                ...
                pdfUrl={previewUrl}
                title="Preview Tanda Terima SiLPA"
            />
        </div>
    );
}
