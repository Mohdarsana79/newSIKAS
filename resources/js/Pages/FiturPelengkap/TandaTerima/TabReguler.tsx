...
export default function TabReguler() {
    ...
    const fetchGenerateCount = async (year: string) => {
        try {
            const response = await axios.get(vroute('api.tanda-terima.check-available'), {
                params: { tahun: year }
            });
    ...
    const fetchYears = async () => {
        try {
            const response = await axios.get(vroute('api.tanda-terima.tahun'));
            if (response.data.success) {
    ...
    const fetchStats = async () => {
        try {
            const responseAvail = await axios.get(vroute('api.tanda-terima.check-available'));
    ...
    const fetchData = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axios.get(vroute('api.tanda-terima.search'), {
                params: {
                    page: page,
                    ...filters
                }
    ...
    const processGenerateBatch = async () => {
        ...
        try {
            ...
            const initialCheck = await axios.get(vroute('api.tanda-terima.check-available'), {
            ...
            while (hasMore) {
                const response = await axios.post(vroute('tanda-terima.generate-batch'), {
                    limit: BATCH_SIZE,
    ...
    const processDeleteAll = async (e: React.FormEvent) => {
        ...
        setProcessing(true);
        try {
            const response = await axios.delete(vroute('tanda-terima.delete-all'));
            if (response.data.success) {
    ...
    const processDownloadAll = (mode: 'current' | 'year') => {
        ...
        window.open(`${vroute('tanda-terima.download-all')}?${params.toString()}`, '_blank');
    ...
    const processPrint = (settings: PrintSettings) => {
        if (selectedTandaTerimaId) {
            const baseUrl = vroute('tanda-terima.pdf', selectedTandaTerimaId);
            const params = new URLSearchParams({
