...
export default function TabKinerja({ auth }: { auth: any }) {
    ...
    const fetchGenerateCount = async (year: string) => {
        try {
            const response = await axios.get(route('kinerja-kwitansi.check-available'), {
                params: { tahun: year }
            });
    ...
    const fetchYears = async () => {
        try {
            const response = await axios.get(route('kinerja-kwitansi.tahun-anggaran'));
            if (response.data.success) {
    ...
    const fetchStats = async () => {
        try {
            const responseAvail = await axios.get(route('kinerja-kwitansi.check-available'));
            ...
            const responseDebug = await axios.get(route('kinerja-kwitansi.debug-count'));
    ...
    const fetchData = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axios.get(route('kinerja-kwitansi.search'), {
                params: {
                    page: page,
                    ...filters
                }
    ...
    const processGenerateBatch = async () => {
        ...
        try {
            ...
            const initialCheck = await axios.get(route('kinerja-kwitansi.check-available'), {
            ...
            while (hasMore) {
                const response = await axios.post(route('kinerja-kwitansi.generate-batch'), {
                    limit: BATCH_SIZE,
    ...
    const processDeleteAll = async (e: React.FormEvent) => {
        ...
        setProcessing(true);
        try {
            const response = await axios.delete(route('kinerja-kwitansi.delete-all'));
            if (response.data.success) {
    ...
    const processDownloadAll = (mode: 'current' | 'year', type: 1 | 2 | 3 | 4) => {
        ...
        let targetUrl = route('kinerja-kwitansi.download-all');
        if (type === 2) targetUrl = route('kinerja-kwitansi.download-all2');
        if (type === 3) targetUrl = route('kinerja-kwitansi.download-all3');
        if (type === 4) targetUrl = route('kinerja-kwitansi.download-all4');
    ...
    const processPrint = (settings: PrintSettings) => {
        if (selectedKwitansiId) {
            let baseUrl = route('kinerja-kwitansi.pdf', selectedKwitansiId);
            if (selectedKwitansiType === 2) baseUrl = route('kinerja-kwitansi.pdf2', selectedKwitansiId);
            if (selectedKwitansiType === 3) baseUrl = route('kinerja-kwitansi.pdf3', selectedKwitansiId);
            if (selectedKwitansiType === 4) baseUrl = route('kinerja-kwitansi.pdf4', selectedKwitansiId);
