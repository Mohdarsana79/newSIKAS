...
export default function TabKinerjaSilpa({ auth }: { auth: any }) {
    ...
    const fetchGenerateCount = async (year: string) => {
        try {
            const response = await axios.get(route('api.kinerja-silpa-kwitansi.check-available'), {
                params: { tahun: year }
            });
            if (response.data.success) {
                setGenerateCount(response.data.data.available_count);
            }
        } catch (error) {
    ...
    const fetchYears = async () => {
        try {
            const response = await axios.get(route('api.kinerja-silpa-kwitansi.tahun'));
            if (response.data.success) {
    ...
    const fetchStats = async () => {
        try {
            const responseAvail = await axios.get(route('api.kinerja-silpa-kwitansi.check-available'));
            if (responseAvail.data.success) {
                setStats(prev => ({
                    ...prev,
                    available: responseAvail.data.data.available_count
                }));
            }
            const responseDebug = await axios.get(route('api.kinerja-silpa-kwitansi.debug-count'));
            if (responseDebug.data.success) {
                setStats(prev => ({
                    ...prev,
                    total: responseDebug.data.data.with_kwitansi
                }));
            }
        } catch (error) {
    ...
    const fetchData = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axios.get(route('api.kinerja-silpa-kwitansi.search'), {
                params: {
                    page: page,
                    ...filters
                }
            ...
            if (response.data.success) {
                setKwitansis(response.data.data);
    ...
    const processGenerateBatch = async () => {
        ...
        try {
            ...
            const initialCheck = await axios.get(route('api.kinerja-silpa-kwitansi.check-available'), {
                params: { tahun: generateYear }
            });
            const totalToProcess = initialCheck.data.data.available_count;
            ...
            while (hasMore) {
                const response = await axios.post(route('kinerja-silpa-kwitansi.generate-batch'), {
                    limit: BATCH_SIZE,
                ...
                // CRITICAL: Offset by failed count to skip them
                ...
                if (processedInBatch < BATCH_SIZE || processedTotal >= totalToProcess + failedTotal) { // Rough check for end
                    if (processedInBatch < BATCH_SIZE) hasMore = false;
                }

                // Secondary safety break
    ...
    const processDeleteAll = async (e: React.FormEvent) => {
        ...
        setProcessing(true);
        try {
            const response = await axios.delete(route('kinerja-silpa-kwitansi.delete-all'));
            if (response.data.success) {
    ...
    const processDownloadAll = (mode: 'current' | 'year', type: 1 | 2 | 3 | 4) => {
        ...
        let targetUrl = route('kinerja-silpa-kwitansi.download-all');
        if (type === 2) targetUrl = route('kinerja-silpa-kwitansi.download-all2');
        if (type === 3) targetUrl = route('kinerja-silpa-kwitansi.download-all3');
        if (type === 4) targetUrl = route('kinerja-silpa-kwitansi.download-all4');
    ...
    const processPrint = (settings: PrintSettings) => {
        if (selectedKwitansiId) {
            let baseUrl = route('kinerja-silpa-kwitansi.pdf', selectedKwitansiId);
            if (selectedKwitansiType === 2) baseUrl = route('kinerja-silpa-kwitansi.pdf2', selectedKwitansiId);
            if (selectedKwitansiType === 3) baseUrl = route('kinerja-silpa-kwitansi.pdf3', selectedKwitansiId);
            if (selectedKwitansiType === 4) baseUrl = route('kinerja-silpa-kwitansi.pdf4', selectedKwitansiId);
    ...
    return (
            ...
            <div className="flex justify-end gap-2 mb-4">
                <button onClick={startGenerateBatch} disabled={stats.available === 0} className={`px-4 py-2 rounded-md text-sm flex items-center gap-2 text-white ${stats.available === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600'}`}>
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.384-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"></path></svg>
                    Generate Otomatis
                </button>
                <button onClick={startDownloadAll} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm flex items-center gap-2">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                    Download All
                </button>
                <button onClick={startDeleteAll} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md text-sm flex items-center gap-2">
            ...
            <div className="space-y-6">
                    ...
                    {/* Filter Section */}
                    <div className="bg-gradient-to-br from-indigo-500 to-purple-600 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 mb-8 shadow-sm flex flex-col md:flex-row gap-4 items-end md:items-center">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow">
                        ...
                        <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex flex-col md:flex-row justify-between items-center gap-4">
                            <div className="text-sm text-gray-500 dark:text-gray-400">
                                Menampilkan {kwitansis.length} dari {pagination.total || 0} data
                            </div>
            ...
            <Modal show={confirmingBatchGenerate} onClose={closeModal}>
                <div className="p-6">
                    ...
                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        Anda akan memproses <strong>{generateCount}</strong> data transaksi BKU menjadi kwitansi secara otomatis untuk tahun anggaran yang dipilih.
                        <br />
            ...
            <Modal show={confirmingDownload} onClose={closeModal}>
                <div className="p-6">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">
                        Download Semua Kwitansi
                    </h2>

                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        Anda akan mengunduh semua data kwitansi berdasarkan filter yang aktif saat ini.
                        <br />
            ...
            <Modal show={confirmingDeleteAll} onClose={closeModal}>
                <form onSubmit={processDeleteAll} className="p-6">
                    <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">
                        Hapus SEMUA Kwitansi
                    </h2>

                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        <strong>PERINGATAN:</strong> Tindakan ini tidak dapat dibatalkan! Semua data kwitansi yang telah digenerate akan dihapus.
                        Data BKU tidak akan terhapus, hanya kwitansinya saja.
                    </p>
