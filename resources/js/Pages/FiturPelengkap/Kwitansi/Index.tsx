...
export default function KwitansiIndex({ auth }: { auth: any }) {
    ...
    const fetchGenerateCount = async (year: string) => {
        try {
            const response = await axios.get(route('api.kwitansi.check-available'), {
                params: { tahun: year }
            });
    ...
    const fetchYears = async () => {
        try {
            const response = await axios.get(route('api.kwitansi.tahun'));
            if (response.data.success) {
    ...
    const fetchStats = async () => {
        try {
            const responseAvail = await axios.get(route('api.kwitansi.check-available'));
            ...
            const responseDebug = await axios.get(route('api.kwitansi.debug-count'));
    ...
    const fetchData = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axios.get(route('api.kwitansi.search'), {
                params: {
                    page: page,
                    ...filters
                }
    ...
    const processGenerateBatch = async () => {
        ...
        try {
            ...
            const initialCheck = await axios.get(route('api.kwitansi.check-available'), {
            ...
            while (hasMore) {
                const response = await axios.post(route('kwitansi.generate-batch'), {
                    limit: BATCH_SIZE,
    ...
    const processDeleteAll = async (e: React.FormEvent) => {
        ...
        setProcessing(true);
        try {
            const response = await axios.delete(route('kwitansi.delete-all'));
            if (response.data.success) {
                ...
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            // Disable if no data available to generate
