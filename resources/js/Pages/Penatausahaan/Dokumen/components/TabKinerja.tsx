...
interface Props {
    auth: any;
    kinerjaPenganggarans: { id: number; tahun_anggaran: string }[];
}

export default function TabKinerja({ auth, kinerjaPenganggarans }: Props) {
    ...
    const fetchData = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axios.get(route('kinerja-api.dokumen.data'), {
                params: {
                    kinerja_penganggaran_id: selectedTahun,
                    bulan: selectedBulan,
                    search: search,
                    page: page
                }
    ...
    const openAddModal = (bku: BukuKasUmum) => {
        ...
        setData({
            kinerja_penganggaran_id: selectedTahun,
            kinerja_buku_kas_umum_id: bku.id.toString(),
            nama_dokumen: '',
            link_drive: ''
        });
        setIsModalOpen(true);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('kinerja-dokumen.store'), {
            onSuccess: () => {
                closeModal();
    ...
    const confirmDelete = () => {
        if (deleteId) {
            router.delete(route('kinerja-dokumen.destroy', deleteId), {
                onSuccess: () => {
                    closeDeleteModal();
                    fetchData(pagination?.current_page || 1);
                }
