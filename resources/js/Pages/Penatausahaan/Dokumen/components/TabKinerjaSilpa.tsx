...
interface Props {
    auth: any;
    kinerjaSilpaPenganggarans: { id: number; tahun_anggaran: string }[];
}

export default function TabKinerjaSilpa({ auth, kinerjaSilpaPenganggarans }: Props) {
    ...
    const fetchData = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axios.get(route('kinerja-silpa-api.dokumen.data'), {
                params: {
                    kinerja_silpa_penganggaran_id: selectedTahun,
                    bulan: selectedBulan,
                    search: search,
                    page: page
                }
    ...
    const openAddModal = (bku: BukuKasUmum) => {
        ...
        setData({
            kinerja_silpa_penganggaran_id: selectedTahun,
            kinerja_silpa_buku_kas_umum_id: bku.id.toString(),
            nama_dokumen: '',
            link_drive: ''
        });
        setIsModalOpen(true);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('kinerja-silpa-dokumen.store'), {
            onSuccess: () => {
                closeModal();
    ...
    const confirmDelete = () => {
        if (deleteId) {
            router.delete(route('kinerja-silpa-dokumen.destroy', deleteId), {
                onSuccess: () => {
                    closeDeleteModal();
                    fetchData(pagination?.current_page || 1);
                }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                            <div>
                                ...
                                <SearchableSelect
                                    ...
                                    options={kinerjaSilpaPenganggarans}
