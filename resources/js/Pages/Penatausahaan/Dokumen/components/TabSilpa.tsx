...
interface Props {
    auth: any;
    silpaPenganggarans: { id: number; tahun_anggaran: string }[];
}

export default function TabSilpa({ auth, silpaPenganggarans }: Props) {
    ...
    const fetchData = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axios.get(route('silpa-api.dokumen.data'), {
                params: {
                    silpa_penganggaran_id: selectedTahun,
                    bulan: selectedBulan,
                    search: search,
                    page: page
                }
    ...
    const openAddModal = (bku: BukuKasUmum) => {
        ...
        setData({
            silpa_penganggaran_id: selectedTahun,
            silpa_buku_kas_umum_id: bku.id.toString(),
            nama_dokumen: '',
            link_drive: ''
        });
        setIsModalOpen(true);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('silpa-dokumen.store'), {
            preserveScroll: true,
            onSuccess: () => {
                setIsModalOpen(false);
                reset();
    ...
    const confirmDelete = () => {
        if (deleteId) {
            router.delete(route('silpa-dokumen.destroy', deleteId), {
                onSuccess: () => {
                    closeDeleteModal();
                    fetchData(pagination?.current_page || 1);
                }
