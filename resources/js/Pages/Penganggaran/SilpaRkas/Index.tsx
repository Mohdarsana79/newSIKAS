...
interface Props extends Record<string, unknown> {
    anggaran: {
        ...
        juni_bku_closed: boolean;
        tahun: string;
        pagu_total: string;
        sisa_total: string;
        sumber_dana: string;
        status: string;
        tahap_1: { periode: string; persen: string; terpakai: string };
        tahap_2: { periode: string; persen: string; terpakai: string };
    };
    items: RkasItem[];
    months: MonthFilter[];
...
export default function Index({ auth, anggaran, items, months, kegiatanOptions, rekeningOptions }: PageProps<Props>) {
    ...
    const handleDetail = async (id: number) => {
        try {
            const response = await axios.get(route('silpa-rkas.getEditData', id));
    ...
    const handleEdit = async (id: number) => {
        try {
            ...
            setData({
                ...
                alokasi: editData.alokasi.map((item: any) => ({
                    ...item,
    ...
    const confirmDelete = () => {
        ...
        router.delete(route('silpa-rkas.destroyGroup'), {
            data: { id: editId },
    ...
    const submit = (e: React.FormEvent) => {
        ...
        if (isEditMode && editId) {
            router.post(route('silpa-rkas.updateGroup'), {
                ...data,
                original_id: editId
            } as any, {
            ...
            post(route('silpa-rkas.store', anggaran.id), {
                onSuccess: () => {
                    setIsModalOpen(false);
                    reset();
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        ...
                        <div className="flex flex-wrap gap-3">
                            <PrimaryButton
                                ...
                                className="bg-teal-600 hover:bg-teal-700 focus:ring-teal-500 flex items-center gap-2"
                            >
                            ...
                            <Link href={route('silpa-rkas.summary', anggaran.id)} className="bg-indigo-600 text-white hover:bg-indigo-700 px-4 py-2 rounded-md text-sm font-medium flex items-center gap-2 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl p-6 text-white shadow-md relative overflow-hidden">
                            <div className="relative z-10">
                                ...
                                <div className="text-sm font-semibold text-indigo-100 mb-2 bg-white/10 inline-block px-3 py-1 rounded-full border border-white/20">
                                    Sisa Total: Rp {anggaran.sisa_total}
                                </div>
                        ...
                        <div className="bg-white border border-gray-100 dark:bg-gray-700 dark:border-gray-600 rounded-xl p-6 shadow-sm relative">
                            ...
                            {/* <div className="flex justify-between items-center text-sm mb-1 mt-4">
                                <span className="text-gray-600 dark:text-gray-400 font-medium">Total Terpakai</span>
                                <span className="text-gray-900 dark:text-white font-bold text-blue-600">Rp {anggaran.tahap_1.terpakai}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm mb-2 mt-1">
                                <span className="text-gray-500 dark:text-gray-400 font-medium text-xs">Sisa Anggaran (Global)</span>
                                <span className="text-gray-900 dark:text-white font-bold text-xs">Rp {anggaran.sisa_total}</span>
                            </div> */}
                            <div className="w-full bg-gray-200 rounded-full h-1.5 mb-4 dark:bg-gray-600">
                                <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: anggaran.tahap_1.persen }}></div>
                            </div>
                            {/* <button
                        ...
                        <div className="bg-white border border-gray-100 dark:bg-gray-700 dark:border-gray-600 rounded-xl p-6 shadow-sm relative">
                            ...
                            <div className="flex justify-between items-center text-sm mb-1 mt-4">
                                <span className="text-gray-600 dark:text-gray-400 font-medium">Total Terpakai</span>
                                <span className="text-gray-900 dark:text-white font-bold text-green-600">Rp {anggaran.tahap_2.terpakai}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm mb-2 mt-1">
                                <span className="text-gray-500 dark:text-gray-400 font-medium text-xs">Sisa Anggaran (Global)</span>
                                <span className="text-gray-900 dark:text-white font-bold text-xs">Rp {anggaran.sisa_total}</span>
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-1.5 mb-4 dark:bg-gray-600">
                                <div className="bg-green-500 h-1.5 rounded-full" style={{ width: anggaran.tahap_2.persen }}></div>
                            </div>
                            {/* <button
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap text-center">
                                                    <Dropdown>
                                                        ...
                                                        <Dropdown.Content contentClasses="py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg">
                                                            {!isBelanja && !anggaran.has_perubahan && (
                                                                <button
                                                                    type="button"
                                                                    onClick={(e) => {
                                                                        e.preventDefault();
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                {data.alokasi.map((alloc, index) => {
                                    ...
                                    const isLunas = hasSpent && spentValue >= qtyValue;
