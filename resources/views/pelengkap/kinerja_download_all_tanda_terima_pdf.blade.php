...
<head>
    <meta charset="utf-8">
    <title>Semua Tanda Terima BOSP Kinerja</title>
    <style>
        /* Folio Landscape */
        @page {
            /* Size handled by controller */
            margin: 1.5cm;
        }
    ...
    <div class="tanda-terima-item">
      <!-- Information Section -->
        <div class="info-header">
            <div>
                <table style="width: 100%; font-size: {{ $fontSize ?? '10pt' }};">
                    <tbody>
                        <tr>
                            <td>Kode Kegiatan</td>
                            <td>:</td>
                        ...
                        <tr>
                            <td>Uraian</td>
                            <td>:</td>
                            <td colspan="2">{{ $item['kinerjaBukuKasUmum']->uraian_opsional ?? '-' }}</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>Sumber Dana</td>
                            <td>:</td>
                            <td colspan="2">BOSP Kinerja</td>
                            <td></td>
                        </tr>
                    </tbody>
                </table>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item['kinerjaBukuKasUmum']->nama_penerima_pembayaran ?? '-' }}</td>
                        <td class="text-center">{{ $item['kinerjaBukuKasUmum']->uraianDetails->first()->volume ?? '-' }}</td>
                        <td class="text-center">{{ $item['kinerjaBukuKasUmum']->uraianDetails->first()->satuan ?? '-' }}</td>
                        <td class="currency" style="text-align: right;">Rp {{
                            number_format($item['kinerjaBukuKasUmum']->uraianDetails->first()->harga_satuan ?? 500000, 0, ',',
                            '.') }}</td>
        ...
        <div class="footer">
            <table class="signature-table">
                <tr>
                    <td style="font-size: {{ $fontSize ?? '10pt' }};">
                        Mengetahui,<br>
                        Kepala Sekolah
                        <div class="signature-name">
                            {{ $item['kinerjaPenganggaran']->kepala_sekolah ?? '-' }}
                        </div>
                        <div class="signature-nip">
                            NIP. {{ $item['kinerjaPenganggaran']->nip_kepala_sekolah ?? '-' }}
                        </div>
                    </td>
                    <td style="font-size: {{ $fontSize ?? '10pt' }};">
                        ...
                        <div class="signature-name">
                            {{ $item['kinerjaPenganggaran']->bendahara ?? '-' }}
                        </div>
                        <div class="signature-nip">
                            NIP. {{ $item['kinerjaPenganggaran']->nip_bendahara ?? '-' }}
                        </div>
                    </td>
                </tr>
            </table>
