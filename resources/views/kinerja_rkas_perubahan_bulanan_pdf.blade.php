...
<html lang="id">
    ...
    <head>
        ...
        <title>Cetak RKAS Perubahan Bulanan</title>
        <style>
            @page {
    ...
    <body>
        @foreach ($rkaBulananData as $currentMonth => $monthData)
            <div style="page-break-after: {{ $loop->last ? 'auto' : 'always' }}">
                <div class="header">
                    <h3 class="uppercase font-bold">KERTAS KERJA RENCANA KEGIATAN DAN ANGGARAN SEKOLAH (RKAS PERUBAHAN)
                        PER BULAN
                        {{ strtoupper($currentMonth) }}</h3>
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="text-right">Rp. {{ number_format($anggaran['pagu_anggaran'], 0, ',', '.') }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th class="text-center" style="width: 100px;">Kode Rekening</th>
                                <th class="text-center" style="width: 80px;">Kode Program</th>
                        ...
                        <tbody>
                            @if (empty($monthData))
                                <tr>
                                    <td colspan="8" class="text-center">Tidak ada data belanja untuk bulan ini.</td>
                                </tr>
                            @else
                                @php $no = 1; @endphp
                                @foreach ($monthData as $programKode => $program)
                                    <!-- Program Row -->
                                    <tr class="bg-orange">
                                        ...
                                        <td class="text-right font-bold">
                                            {{ number_format($program['jumlah'], 0, ',', '.') }}</td>
                                    </tr>
                                            ...
                                            @if (!empty($subProgram['uraian_programs']))
                                                @foreach ($subProgram['uraian_programs'] as $uraianKode => $uraianProgram)
                                                    <tr class="bg-teal">
                                                        ...
                                                        <td class="text-center">-</td>
                                                        <td class="text-right font-bold">
                                                            {{ number_format($uraianProgram['jumlah'], 0, ',', '.') }}
                                                        </td>
                                                    </tr>
                                                    ...
                                                    @if (!empty($uraianProgram['items']))
                                                        @foreach ($uraianProgram['items'] as $item)
                                                            <tr>
                                                                ...
                                                                <td class="text-center">{{ $item['kode_rekening'] }}
                                                                </td>
                                                                <td class="text-center">{{ $subProgramKode }}</td>
                                                                <td>{{ $item['uraian'] }}</td>
                                ...
                                <tr class="bg-grey font-bold" style="border-top: 2px solid black;">
                                    ...
                                    <td class="text-right">
                                        {{ number_format(collect($monthData)->sum('jumlah'), 0, ',', '.') }}
                                    </td>
                                </tr>
                            @endif
                        </tbody>
